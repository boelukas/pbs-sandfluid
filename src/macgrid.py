...
class MacGrid:
    def __init__(self, grid_size:int) -> None:
        ...
        # Cell centered grids
        self.cell_type = ti.field( ti.f32, shape=(self.grid_size, self.grid_size, self.grid_size))
        self.pressure = ti.field( ti.f32, shape=(self.grid_size, self.grid_size, self.grid_size))
        self.divergence = ti.field(ti.f32, shape=(self.grid_size,self.grid_size,self.grid_size))

        # Edge centered grids
        self.v_x = ti.field(ti.f32,shape=(self.grid_size+1,self.grid_size,self.grid_size))
        self.v_y = ti.field(ti.f32,shape=(self.grid_size,self.grid_size+1,self.grid_size))
        self.v_z = ti.field(ti.f32,shape=(self.grid_size,self.grid_size,self.grid_size+1))
        
        self.f_x = ti.field(ti.f32,shape=(self.grid_size+1,self.grid_size,self.grid_size))
        self.f_y = ti.field(ti.f32,shape=(self.grid_size,self.grid_size+1,self.grid_size))
        self.f_z = ti.field(ti.f32,shape=(self.grid_size,self.grid_size,self.grid_size+1))

        self.splat_x_weights = ti.field(ti.f32,shape=(self.grid_size+1,self.grid_size,self.grid_size))
        self.splat_y_weights = ti.field(ti.f32,shape=(self.grid_size,self.grid_size+1,self.grid_size))
        self.splat_z_weights = ti.field(ti.f32,shape=(self.grid_size,self.grid_size,self.grid_size+1))

        # particles
        self.particle_pos = ti.Vector.field(3, ti.f32, shape=(self.grid_size * 2, self.grid_size * 2, self.grid_size * 2))
        self.particle_v = ti.Vector.field(3, ti.f32, shape=(self.grid_size * 2, self.grid_size * 2, self.grid_size * 2))
        self.particle_active = ti.field( ti.f32, shape=(self.grid_size * 2, self.grid_size * 2, self.grid_size * 2))
    ...
    def init_particles(self):
        for i, j, k in self.particle_pos:
            ...
            even_j = j % 2 == 0
            even_k = k % 2 == 0
            if self.cell_type[grid_idx_i, grid_idx_j, grid_idx_k] == CellType.SAND.value:
                self.particle_active[i, j, k] = 1
                particle_pos_x, particle_pos_y, particle_pos_z = 0.0, 0.0, 0.0
                if even_i:
                    particle_pos_x  = float(grid_idx_i) + 0.25 + ((ti.random(int) % 50) - 25)/100.0
                else:
                    particle_pos_x = grid_idx_i + 0.75 + ((ti.random(int) % 50) - 25)/100.0
                if even_j:
                    particle_pos_y  = float(grid_idx_j) + 0.25 + ((ti.random(int) % 50) - 25)/100.0
                else:
                    particle_pos_y = grid_idx_j + 0.75 + ((ti.random(int) % 50) - 25)/100.0
                if even_k:
                    particle_pos_z  = float(grid_idx_k) + 0.25 + ((ti.random(int) % 50) - 25)/100.0
                else:
                    particle_pos_z = grid_idx_k + 0.75 + ((ti.random(int) % 50) - 25)/100.0

                self.particle_pos[i, j, k] = [particle_pos_x,particle_pos_y, particle_pos_z]
            else:
    ...
    def sample(self, grid, x, y, z, x_offset, y_offset, z_offset, x_resolution, y_resolution, z_resolution):
        ...
        x_val_front_down = grid[x_down, y_down, z_down] * (1 - diff_x) + grid[x_up, y_down, z_down] * diff_x
        x_val_back_down = grid[x_down, y_down, z_up] * (1 - diff_x) + grid[x_up, y_down, z_up] * diff_x
        x_val_front_up = grid[x_down, y_up, z_down] * (1 - diff_x) + grid[x_up, y_up, z_down] * diff_x
        x_val_back_up = grid[x_down, y_up, z_up] * (1 - diff_x) + grid[x_up, y_up, z_up] * diff_x
    ...
    def sample_cell_centered(self, grid, x, y, z):
        return self.sample(grid, x, y, z, 0.5, 0.5, 0.5, self.grid_size, self.grid_size, self.grid_size)

    @ti.func
    def sample_x_edged(self, grid, x, y, z):
        return self.sample(grid, x, y, z, 0, 0.5, 0.5,self.grid_size+1, self.grid_size, self.grid_size)
    
    @ti.func
    def sample_y_edged(self, grid, x, y, z):
        return self.sample(grid, x, y, z, 0.5, 0.0, 0.5,self.grid_size, self.grid_size+1, self.grid_size)
    
    @ti.func
    def sample_z_edged(self, grid, x, y, z):
        return self.sample(grid, x, y, z, 0.5, 0.5, 0.0, self.grid_size, self.grid_size, self.grid_size + 1)
    ...
    @ti.func
    def splat(self, target_field, particle_x, particle_y, particle_z, particle_value, weights, x_offset, y_offset, z_offset, x_resolution, y_resolution, z_resolution):
        ...
        target_field[x_down, y_down, z_down] += particle_value * (1 - diff_x) * (1 - diff_y) * (1 - diff_z)
        weights[x_down, y_down, z_down] += (1 - diff_x) * (1 - diff_y) * (1 - diff_z)

        target_field[x_up, y_down, z_down] += particle_value * diff_x * (1 - diff_y) * (1 - diff_z)
        weights[x_up, y_down, z_down] += diff_x * (1 - diff_y) * (1 - diff_z)

        target_field[x_down, y_up, z_down] += particle_value * (1 - diff_x) * diff_y * (1 - diff_z)
        weights[x_down, y_up, z_down] += (1 - diff_x) * diff_y * (1 - diff_z)

        target_field[x_down, y_down, z_up] += particle_value * (1 - diff_x) * (1 - diff_y) * diff_z
        weights[x_down, y_down, z_up] += (1 - diff_x) * (1 - diff_y) * diff_z
        
        target_field[x_up, y_down, z_up] += particle_value * diff_x * (1 - diff_y) * diff_z
        weights[x_up, y_down, z_up] += diff_x * (1 - diff_y) * diff_z

        target_field[x_up, y_up, z_down] += particle_value * diff_x * diff_y * (1 - diff_z)
        weights[x_up, y_up, z_down] += diff_x * diff_y * (1 - diff_z)

        target_field[x_down, y_up, z_up] += particle_value * (1 - diff_x) * diff_y * diff_z
    ...
    def splat_cell_centered(self, grid, x, y, z, value, weights):
        return self.splat(grid, x, y, z, value, weights, 0.5, 0.5, 0.5, self.grid_size, self.grid_size, self.grid_size)

    @ti.func
    def splat_x_edged(self, grid, x, y, z, value):
        return self.splat(grid, x, y, z, value, self.splat_x_weights, 0, 0.5, 0.5,self.grid_size+1, self.grid_size, self.grid_size)
    
    @ti.func
    def splat_y_edged(self, grid, x, y, z, value):
        return self.splat(grid, x, y, z, value, self.splat_y_weights, 0.5, 0.0, 0.5,self.grid_size, self.grid_size+1, self.grid_size)
    
    @ti.func
    def splat_z_edged(self, grid, x, y, z, value):
        return self.splat(grid, x, y, z, value, self.splat_z_weights, 0.5, 0.5, 0.0, self.grid_size, self.grid_size, self.grid_size + 1)  
    ...
    def show_v_y(self):
        ...
        x, y, z = np.meshgrid(np.arange(0, resolution, 1),
