...
class ParticleVisualization(object):
    ...
    def update_particles(self):
        particle_positions = self.mac_grid.particle_pos.to_numpy().reshape(( (self.mac_grid.grid_size * 2)**3, 3))
        filtered_pos = [self.scale * x for x in particle_positions if not np.array_equal(x, np.array([0.0, 0.0, 0.0]))]
