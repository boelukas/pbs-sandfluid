...
class PressureSolver(object):
    ...
    def compute_divergence(self):
        ...
        for x, y, z in ti.ndrange(
            self.mac_grid.grid_size - 1,
            self.mac_grid.grid_size - 1,
            self.mac_grid.grid_size - 1,
        ):
            du_dx = (
                self.mac_grid.velX_grid[x + 1, y, z] - self.mac_grid.velX_grid[x, y, z]
            ) / self.mac_grid.dx
            du_dy = (
                self.mac_grid.velY_grid[x, y + 1, z] - self.mac_grid.velY_grid[x, y, z]
            ) / self.mac_grid.dx
            du_dz = (
                self.mac_grid.velZ_grid[x, y, z + 1] - self.mac_grid.velZ_grid[x, y, z]
            ) / self.mac_grid.dx
    ...
    def project(self, dt: ti.f32):
        DENSITY = 1.0
        for x, y, z in ti.ndrange(
            (1, self.mac_grid.grid_size),
            (1, self.mac_grid.grid_size - 1),
            (1, self.mac_grid.grid_size - 1),
        ):
            self.mac_grid.velX_grid[x, y, z] -= (
                dt
                * (1.0 / DENSITY)
                * (
